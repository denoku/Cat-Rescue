...
import { CalendarIcon, Clock, Heart, SlashIcon, Users } from 'lucide-react'
...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  ...
  if (!cat) {
    return {
      title: 'Cat Not Found | Cat House on the Kings',
    }
  }
  
  return {
    title: `Adopt ${cat.name} | Cat House on the Kings`,
    description: `Meet ${cat.name}, a ${cat.age} ${cat.gender} cat available for adoption. ${cat.description.substring(0, 100)}...`,
    alternates: {
      canonical: `https://cathouseonthekings.com/adopt/${cat.id}`,
    },
    openGraph: {
      ...
      images: [
        {
          url: `https://cathouseonthekings.com${cat.image}`,
          width: 800,
          height: 600,
...
export default function CatDetailsPage({ params }: { params: { id: string } }) {
  ...
  const catDetails = {
    breed: cat.tags.find(tag => tag.includes('tabby') || tag.includes('calico') || tag === 'black') || 'Mixed',
    personality: ['Affectionate', 'Playful', ...(cat.tags.filter(t => t !== 'special needs' && t !== 'bonded pair'))],
    health: 'Vaccinated, neutered/spayed, microchipped, FIV/FeLV negative',
    goodWith: cat.tags.includes('special needs') ? ['Calm environments', 'Adult homes'] : ['Other cats', 'Children', 'Calm dogs'],
    needs: cat.tags.includes('special needs') ? 'Special diet, medication monitoring' : 'Regular playtime, moderate grooming',
        ...
        <div>
          ...
          <div className="flex items-center justify-between text-sm text-gray-500 mb-6">
            <div className="flex items-center">
              ...
              <span>At shelter since {new Date(catDetails.dateArrived).toLocaleDateString()}</span>
            </div>
            <div>ID: {cat.id}</div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <Button className="bg-[#a2bb31] hover:bg-[#8fa82a] text-white flex-1 py-6 text-lg">
              <Link href="/adopt/application" className="w-full block text-center">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          <div className="flex flex-col items-center text-center bg-white p-4 rounded-lg shadow-sm">
            ...
            <p className="text-gray-600 text-sm">
              Submit your adoption application online or in person at our sanctuary
            </p>
          </div>
          ...
          <div className="flex flex-col items-center text-center bg-white p-4 rounded-lg shadow-sm">
            ...
            <p className="text-gray-600 text-sm">
              Visit our sanctuary to meet {cat.name} and ensure you're a good match
            </p>
          </div>
          ...
          <div className="flex flex-col items-center text-center bg-white p-4 rounded-lg shadow-sm">
            ...
            <p className="text-gray-600 text-sm">
              Complete the adoption, pay the fee, and welcome {cat.name} to their forever home
            </p>
          </div>
        </div>
      </section>
